...
uploadReport()
{
    ...
    s3cmd --region=eu-west-2 --server-side-encryption --server-side-encryption-kms-id=${S3_KMS_KEY_ID} --access_key=${S3_ACCESS_KEY} --secret_key=${S3_SECRET_KEY} cp s3://${S3_BUCKET_NAME}/test-reports/perf-${REPORT_DATE_TIME}/content/css s3://${S3_BUCKET_NAME}/test-reports/perf-${REPORT_DATE_TIME}/content/css --content-type="text/css" --metadata-directive="REPLACE" --recursive
